...
out_dir_DESeq2 <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/DESeq2/GAZULx2_water_deficits/" # Define the path for the directory where DESeq2 results will be saved


out_dir_GO <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/DESeq2/GAZULx2_water_deficits/GO_enrichment/" # Define the path for the directory where GO term enrichment results will be saved

data_dir_WW <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/tximport/GAZUL_WW/"

data_dir_LWD <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/tximport/GAZUL_LWD/"

data_dir_TWD <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/tximport/GAZUL_TWD/"
...
GAZUL_WW.tpm.data <- read.csv(file=paste0(data_dir_WW,"GAZUL_WW_tpm.tsv"), sep = "\t")
head(GAZUL_WW.tpm.data)

GAZUL_LWD.tpm.data <- read.csv(file=paste0(data_dir_LWD,"GAZUL_LWD_tpm.tsv"), sep = "\t")
head(GAZUL_LWD.tpm.data)

GAZUL_TWD.tpm.data <- read.csv(file=paste0(data_dir_TWD,"GAZUL_TWD_tpm.tsv"), sep = "\t")
head(GAZUL_TWD.tpm.data)
```
...
GAZUL_WW.tpm.data$WW_average <- (GAZUL_WW.tpm.data[,1] + GAZUL_WW.tpm.data[,2] + GAZUL_WW.tpm.data[,3] + GAZUL_WW.tpm.data[,4] + GAZUL_WW.tpm.data[,5]) / 5 # Calculate the mean for the tpm of the replicates and generate a new column with the name after $ ("WW")

# Create a new table with the calculated means of tpm
colnames(GAZUL_WW.tpm.data)[6] <-  "WW_average" # Define the names of the columns for the new table
tpmData.average.GAZUL_WW <- GAZUL_WW.tpm.data[6] # Create the table choosing the right columns
head(tpmData.average.GAZUL_WW)

# In the table of means, create a new column that indicates the maximum tpm among the time points
tpmData.average.GAZUL_WW$maxtpm <- apply(tpmData.average.GAZUL_WW,1,max)
head(tpmData.average.GAZUL_WW)

# Create a new table filtering the transcripts based on the threshold maxtpm > 0.5
GAZUL_WW_genes_0.5tpm <- tpmData.average.GAZUL_WW[tpmData.average.GAZUL_WW$maxtpm > 0.5,]
dim(tpmData.average.GAZUL_WW)
dim(GAZUL_WW_genes_0.5tpm)

head(GAZUL_WW_genes_0.5tpm)
```

```{r}
# 1.1.2. Prepare the data set of the second condition (long water deficit "LWD").

GAZUL_LWD.tpm.data$LWD_average <- (GAZUL_LWD.tpm.data[,1] + GAZUL_LWD.tpm.data[,2] + GAZUL_LWD.tpm.data[,3] + GAZUL_LWD.tpm.data[,4] + GAZUL_LWD.tpm.data[,5]) / 5 # Calculate the mean for the tpm of the replicates and generate a new column with the name after $ ("WW")

# Create a new table with the calculated means of tpm
colnames(GAZUL_LWD.tpm.data)[6]  <-  "LWD_average" # Define the names of the columns for the new table
tpmData.average.GAZUL_LWD <- GAZUL_LWD.tpm.data[6] # Create the table choosing the right columns
head(tpmData.average.GAZUL_LWD)

# In the table of means, create a new column that indicates the maximum tpm among the time points
tpmData.average.GAZUL_LWD$maxtpm <- apply(tpmData.average.GAZUL_LWD,1,max)
head(tpmData.average.GAZUL_LWD)

# Create a new table filtering the transcripts based on the threshold maxtpm > 0.5
GAZUL_LWD_genes_0.5tpm <- tpmData.average.GAZUL_LWD[tpmData.average.GAZUL_LWD$maxtpm > 0.5,]
dim(tpmData.average.GAZUL_LWD)
dim(GAZUL_LWD_genes_0.5tpm)

head(GAZUL_LWD_genes_0.5tpm)
```

```{r}
# 1.1.3. Prepare the data set of the third condition (terminal water deficit "TWD").

GAZUL_TWD.tpm.data$TWD_average <- (GAZUL_TWD.tpm.data[,1] + GAZUL_TWD.tpm.data[,2] + GAZUL_TWD.tpm.data[,3] + GAZUL_TWD.tpm.data[,4] + GAZUL_TWD.tpm.data[,5]) / 5 # Calculate the mean for the tpm of the replicates and generate a new column with the name after $ ("WW")

# Create a new table with the calculated means of tpm
colnames(GAZUL_TWD.tpm.data)[6] <-  "TWD_average" # Define the names of the columns for the new table
tpmData.average.GAZUL_TWD <- GAZUL_TWD.tpm.data[6] # Create the table choosing the right columns
head(tpmData.average.GAZUL_TWD) 

# In the table of means, create a new column that indicates the maximum tpm among the time points
tpmData.average.GAZUL_TWD$maxtpm <- apply(tpmData.average.GAZUL_TWD,1,max)
head(tpmData.average.GAZUL_TWD)

# Create a new table filtering the transcripts based on the threshold maxtpm > 0.5
GAZUL_TWD_genes_0.5tpm <- tpmData.average.GAZUL_TWD[tpmData.average.GAZUL_TWD$maxtpm > 0.5,]
dim(tpmData.average.GAZUL_TWD)
dim(GAZUL_TWD_genes_0.5tpm)

head(GAZUL_TWD_genes_0.5tpm)
```
...
GAZUL_WW_genes_0.5tpm$gene <- row.names(GAZUL_WW_genes_0.5tpm)
head(GAZUL_WW_genes_0.5tpm)

GAZUL_LWD_genes_0.5tpm$gene <- row.names(GAZUL_LWD_genes_0.5tpm)
head(GAZUL_LWD_genes_0.5tpm)

GAZUL_TWD_genes_0.5tpm$gene <- row.names(GAZUL_TWD_genes_0.5tpm)
head(GAZUL_TWD_genes_0.5tpm)

supra_merge_tpm <- list(GAZUL_WW_genes_0.5tpm, GAZUL_LWD_genes_0.5tpm, GAZUL_TWD_genes_0.5tpm) %>% reduce(full_join, by = "gene")
...
GAZUL_WW.count.data <- read.csv(file = paste0(data_dir_WW,"GAZUL_WW_counts.tsv"), sep = "\t") # Import the table of gene count from the directory of trimm results

GAZUL_WW.count.data.2 <- GAZUL_WW.count.data[,1:5]
head(GAZUL_WW.count.data.2)
dim(GAZUL_WW.count.data.2)

# Create the new data set applying the tpm > 0.5 table 
GAZUL_WW.counts_0.5tpm <- GAZUL_WW.count.data.2[rownames(GAZUL_WW.count.data.2) %in% supra_merge_tpm$gene,]
GAZUL_WW.counts_0.5tpm$gene <- rownames(GAZUL_WW.counts_0.5tpm)
dim(GAZUL_WW.counts_0.5tpm)
head(GAZUL_WW.counts_0.5tpm)
```

```{r}
# 1.2.1 Data set of the second condition (long water deficit "LWD")

GAZUL_LWD.count.data <- read.csv(file = paste0(data_dir_LWD,"GAZUL_LWD_counts.tsv"), sep = "\t") # Import the table of gene count from the directory of trimm results

GAZUL_LWD.count.data.2 <- GAZUL_LWD.count.data[,1:5]
head(GAZUL_LWD.count.data.2)
dim(GAZUL_LWD.count.data.2)

# Create the new data set applying the tpm > 0.5 table 
GAZUL_LWD.counts_0.5tpm <- GAZUL_LWD.count.data.2[rownames(GAZUL_LWD.count.data.2) %in% supra_merge_tpm$gene,]
GAZUL_LWD.counts_0.5tpm$gene <- rownames(GAZUL_LWD.counts_0.5tpm)
head(GAZUL_LWD.counts_0.5tpm)
dim(GAZUL_LWD.counts_0.5tpm)
```

```{r}
# 1.2.2 Data set of the third condition (terminal water deficit "TWD")

GAZUL_TWD.count.data <- read.csv(file = paste0(data_dir_TWD,"GAZUL_TWD_counts.tsv"), sep = "\t") # Import the table of gene count from the directory of trimm results

GAZUL_TWD.count.data.2 <- GAZUL_TWD.count.data[,1:5]
head(GAZUL_TWD.count.data.2)
dim(GAZUL_TWD.count.data.2)

# Create the new data set applying the tpm > 0.5 table 
GAZUL_TWD.counts_0.5tpm <- GAZUL_TWD.count.data.2[rownames(GAZUL_TWD.count.data.2) %in% supra_merge_tpm$gene,]
GAZUL_TWD.counts_0.5tpm$gene <- rownames(GAZUL_TWD.counts_0.5tpm)
head(GAZUL_TWD.counts_0.5tpm)
dim(GAZUL_TWD.counts_0.5tpm)
```
...
total_counts_for_DESeq2 <- list(GAZUL_WW.counts_0.5tpm, GAZUL_LWD.counts_0.5tpm, GAZUL_TWD.counts_0.5tpm) %>% reduce(full_join, by = "gene")
...
water_regime <- c("GAZUL_WW","GAZUL_LWD","GAZUL_TWD") # Create and name the vectors needed. Name the vectors in the same order as the replicates are
...
water_regime <- c(paste0("GAZUL_WW"),paste0("GAZUL",i)) # Create and name the vectors needed. Name the vectors in the same order as the replicates are
CondVector <- rep(water_regime,each=5) # Indicate how many replicates are in each vector
CondVector
...
dds_res <- results(dds_analysis, contrast = c("conditions",paste0("GAZUL",i), paste0("GAZUL_WW")))
  ...
  write.csv(ordered_res_na.rm, file = paste0(out_dir_DESeq2,"GAZUL_WW",i,"_results_all.csv")) # Save all the results for the vulcano plots
  write.csv(ordered_res_na.rm[ordered_res_na.rm$padj<0.05,],file = paste0(out_dir_DESeq2,"GAZUL_WW",i,"_results.csv")) # Save the results whose padj is lower than 0.05
  
  assign(paste("GAZUL_WW",i,".na.rm",sep=""), ordered_res_na.rm[ordered_res_na.rm$padj<0.05,]) # Important the name given here is used below
  
  print(paste("GAZUL_WW",i,".na.rm",sep=""))
  dim(assign(paste("GAZUL_WW",i,".na.rm",sep=""), ordered_res_na.rm[ordered_res_na.rm$padj<0.05,]))

  resLFC <- lfcShrink(dds_analysis, coef=paste0("conditions_GAZUL",i,"_vs_GAZUL_WW"), type="ashr")
...
for (i in water_deficit) {
  ...
  head(get(paste("GAZUL_WW",i,".na.rm",sep="")))
  my_data <- get(paste("GAZUL_WW",i,".na.rm",sep="")) # Get the data saved above
  head(my_data)
...
write.csv(DE_genes, file = paste0(out_dir_DESeq2,"GAZUL","_DE_genes_results.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
resLFC_LWD <- lfcShrink(dds_analysis, coef="conditions_GAZUL_LWD_vs_GAZUL_WW", type="ashr")
resLFC_TWD <- lfcShrink(dds_analysis, coef="conditions_GAZUL_TWD_vs_GAZUL_WW", type="apeglm")
...
gene_lengths <- read.csv(file = paste0(data_dir_WW,"GAZUL_WW_gene_length.csv"), header =T) # Import the file with the tximport gene lengths
head(gene_lengths)
...
for (i in water_deficit) {
  ...
  for (j in c("upreg2fold_0.05","downreg2fold_0.05","upreg2fold_0.01","downreg2fold_0.01","upreg2fold_0.001","downreg2fold_0.001")) { # Indicate the name of the (j) items as they were defined just above
    ...
    write.table(GO.wall[GO.wall$over_rep_padj<0.05,], file = paste0(out_dir_GO,"GAZUL",i,"_vs_WW_results_",j,"_GOseq.tsv", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = F)
...
write.csv(file=paste0(out_dir_GO,"GAZUL_GO_enrichment_padj0.05.csv"), merged_GO_data_spread_0.05, row.names = F)

write.csv(file=paste0(out_dir_GO,"GAZUL_GO_enrichment_padj0.01.csv"), merged_GO_data_spread_0.01, row.names = F)

write.csv(file=paste0(out_dir_GO,"GAZUL_GO_enrichment_padj0.001.csv"), merged_GO_data_spread_0.001, row.names = F)
...
deg.LWD <- read.csv(file=paste0(out_dir_DESeq2,"GAZUL_WW_LWD_results.csv"), sep = ",")
...
deg.TWD <- read.csv(file=paste0(out_dir_DESeq2,"GAZUL_WW_TWD_results.csv"), sep = ",")
...
GOterm.up_LWD <- read.csv(file=paste0(out_dir_GO,"GAZUL_LWD_vs_WW_results_upreg2fold_0.001_GOseq.tsv"), sep = "\t")
GOterm.down_LWD <- read.csv(file=paste0(out_dir_GO,"GAZUL_LWD_vs_WW_results_downreg2fold_0.001_GOseq.tsv"), sep = "\t")

# Import GO terms in TWD vs WW conditions, with padj < 0.001
GOterm.up_TWD <- read.csv(file=paste0(out_dir_GO,"GAZUL_TWD_vs_WW_results_upreg2fold_0.001_GOseq.tsv"), sep = "\t")
GOterm.down_TWD <- read.csv(file=paste0(out_dir_GO,"GAZUL_TWD_vs_WW_results_downreg2fold_0.001_GOseq.tsv"), sep = "\t")
...
write.csv(GOterm.up_LWD_ids, file = paste0(out_dir_GO,"GAZUL_LWD_GOterm_upreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
write.csv(GOterm.down_LWD_ids, file = paste0(out_dir_GO,"GAZUL_LWD_GOterm_downreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
write.csv(GOterm.up_TWD_ids, file = paste0(out_dir_GO,"GAZUL_TWD_GOterm_upreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
write.csv(GOterm.down_TWD_ids, file = paste0(out_dir_GO,"GAZUL_TWD_GOterm_downreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
