...
my $input_for_trim = "/jic/scratch/groups/Philippa-Borrill/Ismael/data-trimmed/inputs/trimm_input_HT41_read1.txt"; # Path for the list.txt of samples itself
...
SLURM
 ...
 my $tmp_file = "$output_dir/tmp/trim.$sample"; # Previously, a folder with the name "tmp" needs to be created inside the output folder
  ...
  print SLURM "source package 50fcf79b-73a3-4f94-9553-5ed917823423\n";
	...
	print SLURM "trimmomatic PE -phred33 -threads 4 $f1 $f2 $output_dir/$sample/$sample"."_1.paired.fq.gz $output_dir/$sample/$sample"."_1.unpaired.fq.gz $output_dir/$sample/$sample"."_2.paired.fq.gz $output_dir/$sample/$sample"."_2.unpaired.fq.gz ILLUMINACLIP:$adapter_dir/TruSeq3-PE.fa:2:30:10 LEADING:3 SLIDINGWINDOW:4:15 MINLEN:80\n";
