...
out_dir_DESeq2 <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/DESeq2/REGALLOx2_water_deficits/" # Define the path for the directory where DESeq2 results will be saved


out_dir_GO <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/DESeq2/REGALLOx2_water_deficits/GO_enrichment/" # Define the path for the directory where GO term enrichment results will be saved

data_dir_WW <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/tximport/REGALLO_WW/"

data_dir_LWD <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/tximport/REGALLO_LWD/"

data_dir_TWD <- "//jic-hpc-data/Group-Scratch/Philippa-Borrill/Ismael/tximport/REGALLO_TWD/"
...
REGALLO_WW.tpm.data <- read.csv(file=paste0(data_dir_WW,"REGALLO_WW_tpm.tsv"), sep = "\t")
head(REGALLO_WW.tpm.data)

REGALLO_LWD.tpm.data <- read.csv(file=paste0(data_dir_LWD,"REGALLO_LWD_tpm.tsv"), sep = "\t")
head(REGALLO_LWD.tpm.data)

REGALLO_TWD.tpm.data <- read.csv(file=paste0(data_dir_TWD,"REGALLO_TWD_tpm.tsv"), sep = "\t")
head(REGALLO_TWD.tpm.data)
```
...
REGALLO_WW.tpm.data$WW_average <- (REGALLO_WW.tpm.data[,1] + REGALLO_WW.tpm.data[,2] + REGALLO_WW.tpm.data[,3] + REGALLO_WW.tpm.data[,4] + REGALLO_WW.tpm.data[,5]) / 5 # Calculate the mean for the tpm of the replicates and generate a new column with the name after $ ("WW")

# Create a new table with the calculated means of tpm
colnames(REGALLO_WW.tpm.data)[6] <-  "WW_average" # Define the names of the columns for the new table
tpmData.average.REGALLO_WW <- REGALLO_WW.tpm.data[6] # Create the table choosing the right columns
head(tpmData.average.REGALLO_WW)

# In the table of means, create a new column that indicates the maximum tpm among the time points
tpmData.average.REGALLO_WW$maxtpm <- apply(tpmData.average.REGALLO_WW,1,max)
head(tpmData.average.REGALLO_WW)

# Create a new table filtering the transcripts based on the threshold maxtpm > 0.5
REGALLO_WW_genes_0.5tpm <- tpmData.average.REGALLO_WW[tpmData.average.REGALLO_WW$maxtpm > 0.5,]
dim(tpmData.average.REGALLO_WW)
dim(REGALLO_WW_genes_0.5tpm)

head(REGALLO_WW_genes_0.5tpm)
```

```{r}
# 1.1.2. Prepare the data set of the second condition (long water deficit "LWD").

REGALLO_LWD.tpm.data$LWD_average <- (REGALLO_LWD.tpm.data[,1] + REGALLO_LWD.tpm.data[,2] + REGALLO_LWD.tpm.data[,3] + REGALLO_LWD.tpm.data[,4] + REGALLO_LWD.tpm.data[,5]) / 5 # Calculate the mean for the tpm of the replicates and generate a new column with the name after $ ("WW")

# Create a new table with the calculated means of tpm
colnames(REGALLO_LWD.tpm.data)[6]  <-  "LWD_average" # Define the names of the columns for the new table
tpmData.average.REGALLO_LWD <- REGALLO_LWD.tpm.data[6] # Create the table choosing the right columns
head(tpmData.average.REGALLO_LWD)

# In the table of means, create a new column that indicates the maximum tpm among the time points
tpmData.average.REGALLO_LWD$maxtpm <- apply(tpmData.average.REGALLO_LWD,1,max)
head(tpmData.average.REGALLO_LWD)

# Create a new table filtering the transcripts based on the threshold maxtpm > 0.5
REGALLO_LWD_genes_0.5tpm <- tpmData.average.REGALLO_LWD[tpmData.average.REGALLO_LWD$maxtpm > 0.5,]
dim(tpmData.average.REGALLO_LWD)
dim(REGALLO_LWD_genes_0.5tpm)

head(REGALLO_LWD_genes_0.5tpm)
```

```{r}
# 1.1.3. Prepare the data set of the third condition (terminal water deficit "TWD").

REGALLO_TWD.tpm.data$TWD_average <- (REGALLO_TWD.tpm.data[,1] + REGALLO_TWD.tpm.data[,2] + REGALLO_TWD.tpm.data[,3] + REGALLO_TWD.tpm.data[,4] + REGALLO_TWD.tpm.data[,5]) / 5 # Calculate the mean for the tpm of the replicates and generate a new column with the name after $ ("WW")

# Create a new table with the calculated means of tpm
colnames(REGALLO_TWD.tpm.data)[6] <-  "TWD_average" # Define the names of the columns for the new table
tpmData.average.REGALLO_TWD <- REGALLO_TWD.tpm.data[6] # Create the table choosing the right columns
head(tpmData.average.REGALLO_TWD) 

# In the table of means, create a new column that indicates the maximum tpm among the time points
tpmData.average.REGALLO_TWD$maxtpm <- apply(tpmData.average.REGALLO_TWD,1,max)
head(tpmData.average.REGALLO_TWD)

# Create a new table filtering the transcripts based on the threshold maxtpm > 0.5
REGALLO_TWD_genes_0.5tpm <- tpmData.average.REGALLO_TWD[tpmData.average.REGALLO_TWD$maxtpm > 0.5,]
dim(tpmData.average.REGALLO_TWD)
dim(REGALLO_TWD_genes_0.5tpm)

head(REGALLO_TWD_genes_0.5tpm)
...
REGALLO_WW_genes_0.5tpm$gene <- row.names(REGALLO_WW_genes_0.5tpm)
head(REGALLO_WW_genes_0.5tpm)

REGALLO_LWD_genes_0.5tpm$gene <- row.names(REGALLO_LWD_genes_0.5tpm)
head(REGALLO_LWD_genes_0.5tpm)

REGALLO_TWD_genes_0.5tpm$gene <- row.names(REGALLO_TWD_genes_0.5tpm)
head(REGALLO_TWD_genes_0.5tpm)

supra_merge_tpm <- list(REGALLO_WW_genes_0.5tpm, REGALLO_LWD_genes_0.5tpm, REGALLO_TWD_genes_0.5tpm) %>% reduce(full_join, by = "gene")
...
REGALLO_WW.count.data <- read.csv(file = paste0(data_dir_WW,"REGALLO_WW_counts.tsv"), sep = "\t") # Import the table of gene count from the directory of trimm results

REGALLO_WW.count.data.2 <- REGALLO_WW.count.data[,1:5]
head(REGALLO_WW.count.data.2)
dim(REGALLO_WW.count.data.2)

# Create the new data set applying the tpm > 0.5 table 
REGALLO_WW.counts_0.5tpm <- REGALLO_WW.count.data.2[rownames(REGALLO_WW.count.data.2) %in% supra_merge_tpm$gene,]
REGALLO_WW.counts_0.5tpm$gene <- rownames(REGALLO_WW.counts_0.5tpm)
dim(REGALLO_WW.counts_0.5tpm)
head(REGALLO_WW.counts_0.5tpm)
```

```{r}
# 1.2.1 Data set of the second condition (long water deficit "LWD")

REGALLO_LWD.count.data <- read.csv(file = paste0(data_dir_LWD,"REGALLO_LWD_counts.tsv"), sep = "\t") # Import the table of gene count from the directory of trimm results

REGALLO_LWD.count.data.2 <- REGALLO_LWD.count.data[,1:5]
head(REGALLO_LWD.count.data.2)
dim(REGALLO_LWD.count.data.2)

# Create the new data set applying the tpm > 0.5 table 
REGALLO_LWD.counts_0.5tpm <- REGALLO_LWD.count.data.2[rownames(REGALLO_LWD.count.data.2) %in% supra_merge_tpm$gene,]
REGALLO_LWD.counts_0.5tpm$gene <- rownames(REGALLO_LWD.counts_0.5tpm)
head(REGALLO_LWD.counts_0.5tpm)
dim(REGALLO_LWD.counts_0.5tpm)
```

```{r}
# 1.2.2 Data set of the third condition (terminal water deficit "TWD")

REGALLO_TWD.count.data <- read.csv(file = paste0(data_dir_TWD,"REGALLO_TWD_counts.tsv"), sep = "\t") # Import the table of gene count from the directory of trimm results

REGALLO_TWD.count.data.2 <- REGALLO_TWD.count.data[,1:5]
head(REGALLO_TWD.count.data.2)
dim(REGALLO_TWD.count.data.2)

# Create the new data set applying the tpm > 0.5 table 
REGALLO_TWD.counts_0.5tpm <- REGALLO_TWD.count.data.2[rownames(REGALLO_TWD.count.data.2) %in% supra_merge_tpm$gene,]
REGALLO_TWD.counts_0.5tpm$gene <- rownames(REGALLO_TWD.counts_0.5tpm)
head(REGALLO_TWD.counts_0.5tpm)
dim(REGALLO_TWD.counts_0.5tpm)
```
...
total_counts_for_DESeq2 <- list(REGALLO_WW.counts_0.5tpm, REGALLO_LWD.counts_0.5tpm, REGALLO_TWD.counts_0.5tpm) %>% reduce(full_join, by = "gene")
...
water_regime <- c("REGALLO_WW","REGALLO_LWD","REGALLO_TWD") # Create and name the vectors needed. Name the vectors in the same order as the replicates are
...
dds$conditions <- relevel(dds$conditions, "REGALLO_WW") # Define which of the conditions is the reference
dim(dds)
dds
...
plot <- ggplot(pcaData, aes(x = PC1, y = PC2, color = conditions, label = name)) +
  ...
  ggtitle("REGALLO under two water deficits ") +
...
LWD_merge_tpm <- list(REGALLO_WW_genes_0.5tpm, REGALLO_LWD_genes_0.5tpm) %>% reduce(full_join, by = "gene")
row.names(LWD_merge_tpm) <- LWD_merge_tpm$gene
LWD_merge_tpm <- LWD_merge_tpm[!grepl("LC",LWD_merge_tpm$gene), ]
dim(LWD_merge_tpm)

REGALLO_WW_by_LWD.counts_0.5tpm <- REGALLO_WW.count.data.2[rownames(REGALLO_WW.count.data.2) %in% LWD_merge_tpm$gene,]
REGALLO_WW_by_LWD.counts_0.5tpm$gene <- rownames(REGALLO_WW_by_LWD.counts_0.5tpm)

REGALLO_LWD.counts_0.5tpm <- REGALLO_LWD.count.data.2[rownames(REGALLO_LWD.count.data.2) %in% LWD_merge_tpm$gene,]
REGALLO_LWD.counts_0.5tpm$gene <- rownames(REGALLO_LWD.counts_0.5tpm)

counts_for_DESeq2_LWD <- list(REGALLO_WW_by_LWD.counts_0.5tpm, REGALLO_LWD.counts_0.5tpm) %>% reduce(full_join, by = "gene")
...
TWD_merge_tpm <- list(REGALLO_WW_genes_0.5tpm, REGALLO_TWD_genes_0.5tpm) %>% reduce(full_join, by = "gene")
row.names(TWD_merge_tpm) <- TWD_merge_tpm$gene
TWD_merge_tpm <- TWD_merge_tpm[!grepl("LC",TWD_merge_tpm$gene), ]
dim(TWD_merge_tpm)

REGALLO_WW_by_TWD.counts_0.5tpm <- REGALLO_WW.count.data.2[rownames(REGALLO_WW.count.data.2) %in% TWD_merge_tpm$gene,]
REGALLO_WW_by_TWD.counts_0.5tpm$gene <- rownames(REGALLO_WW_by_TWD.counts_0.5tpm)

REGALLO_TWD.counts_0.5tpm <- REGALLO_TWD.count.data.2[rownames(REGALLO_TWD.count.data.2) %in% TWD_merge_tpm$gene,]
REGALLO_TWD.counts_0.5tpm$gene <- rownames(REGALLO_TWD.counts_0.5tpm)

counts_for_DESeq2_TWD <- list(REGALLO_WW.counts_0.5tpm, REGALLO_TWD.counts_0.5tpm) %>% reduce(full_join, by = "gene") %>% na.omit()
...
water_regime <- c(paste0("REGALLO_WW"),paste0("REGALLO",i)) # Create and name the vectors needed. Name the vectors in the same order as the replicates are
CondVector <- rep(water_regime,each=5) # Indicate how many replicates are in each vector
CondVector
...
plot <- ggplot(pcaData, aes(x = PC1, y = PC2, color = conditions, label = name)) +
  ...
  ggtitle("REGALLO under one treatment") +
  geom_label(aes(label = name), position = nudge, size = 3, size.unit = "mm")
print(plot)
...
dds_res <- results(dds_analysis, contrast = c("conditions",paste0("REGALLO",i), paste0("REGALLO_WW")))
  ...
  write.csv(ordered_res_na.rm, file = paste0(out_dir_DESeq2,"REGALLO_WW",i,"_results_all.csv")) # Save all the results for the vulcano plots
  write.csv(ordered_res_na.rm[ordered_res_na.rm$padj<0.05,],file = paste0(out_dir_DESeq2,"REGALLO_WW",i,"_results.csv")) # Save the results whose padj is lower than 0.05
  
  assign(paste("REGALLO_WW",i,".na.rm",sep=""), ordered_res_na.rm[ordered_res_na.rm$padj<0.05,]) # Important the name given here is used below
  
  print(paste("REGALLO_WW",i,".na.rm",sep=""))
  dim(assign(paste("REGALLO_WW",i,".na.rm",sep=""), ordered_res_na.rm[ordered_res_na.rm$padj<0.05,]))

  resLFC <- lfcShrink(dds_analysis, coef=paste0("conditions_REGALLO",i,"_vs_REGALLO_WW"), type="ashr")
...
for (i in water_deficit) {
  ...
  head(get(paste("REGALLO_WW",i,".na.rm",sep="")))
  my_data <- get(paste("REGALLO_WW",i,".na.rm",sep="")) # Get the data saved above
  head(my_data)
...
write.csv(DE_genes, file = paste0(out_dir_DESeq2,"REGALLO","_DE_genes_results.csv")) # Save the data frame/table created with the proper name in .csv format
...
resLFC_LWD <- lfcShrink(dds_analysis, coef="conditions_REGALLO_LWD_vs_REGALLO_WW", type="ashr")
resLFC_TWD <- lfcShrink(dds_analysis, coef="conditions_REGALLO_TWD_vs_REGALLO_WW", type="apeglm")
...
gene_lengths <- read.csv(file = paste0(data_dir_WW,"REGALLO_WW_gene_length.csv"), header =T) # Import the file with the tximport gene lengths
head(gene_lengths)
...
for (i in water_deficit) {
  ...
  head(get(paste("REGALLO_WW",i,".na.rm",sep = ""))) # Define in in DESeq2 #2.2.2. chunk, for each time point in the item list
  my_data <- get(paste("REGALLO_WW",i,".na.rm",sep = "")) # Get the data from DESeq2 analysis
  head(my_data)
  ...
  for (j in c("upreg2fold_0.05","downreg2fold_0.05","upreg2fold_0.01","downreg2fold_0.01","upreg2fold_0.001","downreg2fold_0.001")) { # Indicate the name of the (j) items as they were defined just above
    ...
    write.table(GO.wall[GO.wall$over_rep_padj<0.05,], file = paste0(out_dir_GO,"REGALLO",i,"_vs_WW_results_",j,"_GOseq.tsv", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = F)
...
write.csv(file=paste0(out_dir_GO,"REGALLO_GO_enrichment_padj0.05.csv"), merged_GO_data_spread_0.05, row.names = F)

write.csv(file=paste0(out_dir_GO,"REGALLO_GO_enrichment_padj0.01.csv"), merged_GO_data_spread_0.01, row.names = F)

write.csv(file=paste0(out_dir_GO,"REGALLO_GO_enrichment_padj0.001.csv"), merged_GO_data_spread_0.001, row.names = F)
...
deg.LWD <- read.csv(file=paste0(out_dir_DESeq2,"REGALLO_WW_LWD_results.csv"), sep = ",")
...
deg.TWD <- read.csv(file=paste0(out_dir_DESeq2,"REGALLO_WW_TWD_results.csv"), sep = ",")
...
GOterm.up_LWD <- read.csv(file=paste0(out_dir_GO,"REGALLO_LWD_vs_WW_results_upreg2fold_0.001_GOseq.tsv"), sep = "\t")
GOterm.down_LWD <- read.csv(file=paste0(out_dir_GO,"REGALLO_LWD_vs_WW_results_downreg2fold_0.001_GOseq.tsv"), sep = "\t")

# Import GO terms in TWD vs WW conditions, with padj < 0.001
GOterm.up_TWD <- read.csv(file=paste0(out_dir_GO,"REGALLO_TWD_vs_WW_results_upreg2fold_0.001_GOseq.tsv"), sep = "\t")
GOterm.down_TWD <- read.csv(file=paste0(out_dir_GO,"REGALLO_TWD_vs_WW_results_downreg2fold_0.001_GOseq.tsv"), sep = "\t")
...
write.csv(GOterm.up_LWD_ids, file = paste0(out_dir_GO,"REGALLO_LWD_GOterm_upreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
write.csv(GOterm.down_LWD_ids, file = paste0(out_dir_GO,"REGALLO_LWD_GOterm_downreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
write.csv(GOterm.up_TWD_ids, file = paste0(out_dir_GO,"REGALLO_TWD_GOterm_upreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
...
write.csv(GOterm.down_TWD_ids, file = paste0(out_dir_GO,"REGALLO_TWD_GOterm_downreg_gene_list.csv")) # Save the data frame/table created with the proper name in .csv format
```
